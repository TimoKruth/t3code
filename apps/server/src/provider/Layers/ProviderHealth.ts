...
function isCommandMissingCause(error: unknown, commandName?: string): boolean {
  ...
  return (
    (normalizedCommandName !== undefined &&
      lower.includes(`command not found: ${normalizedCommandName}`)) ||
    (normalizedCommandName !== undefined &&
      lower.includes(`spawn ${normalizedCommandName} enoent`)) ||
...
const runClaudeCodeCommand = (args: ReadonlyArray<string>) => runCommand("claude", args);
...
> = Effect.gen(function* () {
  ...
  return {
    ...
    checkedAt,
    message:
      "Claude Code CLI is reachable. Authentication and runtime readiness are verified on first turn.",
  } satisfies ServerProviderStatus;
});
